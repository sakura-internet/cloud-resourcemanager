...
resource "sakuracloud_vpc_router_port_forwarding" "forward_rules" {
  ...
  protocol        = local.reverse_nat_targets[count.index]["protocol"]
  private_address = local.server_ip
  global_port     = local.reverse_nat_targets[count.index]["port"]
  private_port    = local.reverse_nat_targets[count.index]["port"]
}
